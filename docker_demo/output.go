...
func (t tableOutput) PrintHeader() {
	header := "%-16s %-6s %-6s %-36s %-6s %3s %s\n"
	args := []interface{}{"PCOMM", "PID", "Pid_NS", "UTS_NAME", "PPID", "RET", "ARGS"}
	if t.timestamp {
		header = "%-8s" + header
		args = []interface{}{"TIME(s)", "PCOMM", "PID", "Pid_NS", "UTS_NAME", "PPID", "RET", "ARGS"}
	}
...
func (t tableOutput) PrintLine(e eventPayload) {
	...
	if e.Pid_NS!=1 {
	//fmt.Println(" this is in case  Pid_NS is not 1 aktiwari this is your code running \n")
	} else  {

	fmt.Println(" this is in case  Pid_NS is 1 aktiwari this is your code running \n")
	}
	header := "%-16s %-6d %-6d %-36s %-6s %3d %s\n"
	args := []interface{}{e.Comm, e.Pid, e.Pid_NS, e.UtsName, e.Ppid, e.RetVal, e.Argv}
	if t.timestamp {
		header = "%-8.3f" + header
